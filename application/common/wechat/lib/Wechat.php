...
class Wechat {
    ...
    $this->access_token_file = $options['token_file'];
    $this->jsapi_ticket_file = $options['jsapi_ticket_file'];
	 ...
	 * @param  boolean $param_type true-数字形式的二维码参数，false-字符串形式的二维码参数 
	 * @return string|boolean    二维码图片地址
	 * @author 、小陈叔叔 <cjango@163.com>
	 */
	public function getQRUrl($scene_id = '', $limit = true, $expire = 1800, $param_type = true) {
		if (!isset($this->ticket)) {
			if (!$this->qrcode($scene_id, $limit, $expire, $param_type)) return false;
		}
	 ...
	 * @author 、小陈叔叔 <cjango@163.com>
	 */
	private function qrcode($scene_id = '', $limit = true, $expire = 1800, $param_type = true) {
		if ($param_type) {
			if ($limit && (empty($scene_id) || !is_numeric($scene_id) || $scene_id > 100000 || $scene_id < 1)) {
				$this->error = '场景值必须是1-100000之间的整数';
				return false;
			}
			$params['action_name'] = $limit?'QR_LIMIT_SCENE':'QR_SCENE';
		} else {
			$str_len = strlen($scene_id);
			if (empty($str_len) || $str_len > 64 || $str_len < 1) {
				$this->error = '场景值必须在长度为1-64之间的字符串';
				return false;
			}
			$params['action_name'] = $limit?'QR_LIMIT_STR_SCENE':'QR_STR_SCENE';
		}
	 ...
	 * @author 、小陈叔叔 <cjango@163.com>
	 */
	private function json_encode($arr = array()) {
	...
	public function media_get($media_id) {
		...
		$jsonStr = $this->http(self::MEDIA_GET_URL, $params);
		//$jsonArr = $this->parseJson($jsonStr);
		if ($jsonStr) {
			return $jsonStr;
		}else {
